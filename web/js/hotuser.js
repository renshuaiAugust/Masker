...
xhr1.onreadystatechange = function () {
    if (xhr1.readyState == 4 && (xhr1.status == 200 || xhr1.status == 200)) {
        ...
        for (var i in json1) {
            ...
            string += ' <img src='+json1[i].masker_user_x4+' alt="头像" class="userhead">';
            string += ' <div class="user">';
            string += '<div>用户名:<span class="name">'+json1[i].masker_user_x2+'</span></div>';
            string += ' <div >性别:'+json1[i].masker_user_x5+'</div>';
            string += '<div>出生日期:'+json1[i].masker_user_x6+' </div>';
            string += '<div>粉丝数:	'+json1[i].masker_user_x7+'</div>';
            string += ' <div class="hisaction">动态数:	'+json1[i].masker_user_x8+'</div>';
            string += '</div>';
            if(json1[i].masker_user_x  =='true') {
                string += '<i class="fa fa-star fa-5x star"></i>';
            }else{
                string += '<i class="fa fa-star-o fa-5x star"></i>'
            }
            string += '<div class="intr" id="intr">';
            string += ' <div>个人简介:<br><br><br>'+json1[i].masker_user_x9+'</div>';
            string += '</div>';
            string += ' </div>';
...
$(document).on('click','.star',function(){
    if(this.className=='fa fa-star-o fa-5x star')
    {
        this.className='fa fa-star fa-5x star'
    }else{
        this.className='fa fa-star-o fa-5x star'
    }
    var xhr1 = new XMLHttpRequest();
    xhr1.open('post', "/ServletFollow");
    xhr1.setRequestHeader("Content-Type", "application/x-www-form-urlencoded;");

    xhr1.send("name="+($(this).parent().find('.user').find('.name').html()));
...
// 点击用户动态数跳转到他的动态页
$('.hisaction').on('click',function(){
    window.location.href="../html/hisAction.html"
})
