...
xhr1.onreadystatechange = function () {
    if (xhr1.readyState == 4 && (xhr1.status == 200 || xhr1.status == 200)) {
        ...
        var string = "<a href=''></a>";
        for (var i in json1) {
            string += '<div class="focusperson" id="focusperson">';
            string += ' <img src='+json1.username+' alt="头像" class="userhead">';
            string += ' <div class="user">';
            string += '<div>用户名:'+json1.username+'</div>';
            string += ' <div >性别:'+json1.username+'</div>';
            string += '<div>出生日期:'+json1.username+' </div>';
            string += '<div>粉丝数:	'+json1.username+'</div>';
            string += ' <div class="hisaction">动态数:	'+json1.username+'</div>';
            string += '</div>';
            string += '<i class="fa fa-star fa-5x star"></i>';
            string += '<div class="intr" id="intr">';
            string += ' <div>个人简介:<br><br><br>'+json1.username+'</div>';
            string += '</div>';
            string += ' </div>';
